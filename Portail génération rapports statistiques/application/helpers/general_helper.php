...
function getGUID() {
    if (function_exists('com_create_guid')) {
        ...
        // On ne génère pas les accolades, car cela pose problème ensuite dans le cas du passage de GUID en paramètre d'URL
...
function generation_password(){
    ...
    $password_minuscules = array();
    $longueur_minuscules = strlen($minuscules) - 1;
    for ($i = 0; $i < 3; $i++)
    {
        $n = rand(0, $longueur_minuscules);
        $password_minuscules[] = $minuscules[$n];
    }
    $minus = implode($password_minuscules);
    
    $majuscules = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';    
    $password_majuscules = array();
    $longueur_majuscules = strlen($majuscules) - 1;
    for ($i = 0; $i < 3; $i++)
    {
        $n = rand(0, $longueur_majuscules);
        $password_majuscules[] = $majuscules[$n];
    }
    $majus = implode($password_majuscules);
    
    $chiffres = '0123456789';    
    $password_chiffres = array();
    $longueur_chiffres = strlen($chiffres) - 1;
    for ($i = 0; $i < 2; $i++)
    {
        $n = rand(0, $longueur_chiffres);
        $password_chiffres[] = $chiffres[$n];
    }
    $chiff = implode($password_chiffres);
    
    return str_shuffle($minus.$majus.$chiff);
}