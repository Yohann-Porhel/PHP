...
class Admin_Utilisateur_Controller extends MY_Controller_Administrateur {
    ...
    public function gestionUtilisateur() {
        ...
        $this->form_validation->set_rules('ut_password', 'Mot de passe', 'trim');

        //Récupération des éléments saisis par l'utilisateur
        $ut_email = $this->input->post('ut_email');
        $this->controleFormatMail($ut_email);
        if (empty($this->input->post('ut_password'))) {
            $ut_password_en_clair = generation_password();
        } else {
        ...
        if (empty($this->input->post('ut_bo_password'))) {
            $ut_bo_password = generation_password();
        } else {
    ...
    /* Permet d'afficher le formulaire pré-remplit pour la modification de l'utilisateur */
