...
function js_dialogue_desabonnement(libelleDialogue, id) {

    $("#popupdesabonnement").text(libelleDialogue);
    $("#popupdesabonnement").dialog({
        width: 385,
        buttons: [
            {
                text: "Annuler",
            ...
            {
                text: "Confirmer le désabonnement au rapport",
                click: function (event) {
                    event.preventDefault();
                    var url = 'apobi_client_abonnement_model/insererDateDesabonnement'
                    var donnees = 'fk_rapport=' + id;
                    var posting = $.post(url, donnees);
                    posting.done(function (data) {
                        var content = $(data).find("#content");
                        $("#msg").empty().append(content);
